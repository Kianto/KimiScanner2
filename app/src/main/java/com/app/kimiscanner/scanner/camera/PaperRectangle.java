...
public class PaperRectangle extends View {
    ...
    public void onCornersDetected(Corners corners) {
        ...
        Point temp = corners.cornerPoints.get(0);
        if (temp == null) {
            temp = new Point();
        }
        this.tl = temp;

        temp = corners.cornerPoints.get(1);
        if (temp == null) {
            temp = new Point();
        }
        this.tr = temp;

        temp = corners.cornerPoints.get(2);
        if (temp == null) {
            temp = new Point();
        }
        this.br = temp;

        temp = corners.cornerPoints.get(3);
        if (temp == null) {
            temp = new Point();
        }
        this.bl = temp;
    ...
    public void onCorners2Crop(Corners corners, Size size) {
        ...
        if (corners.cornerPoints != null) {
            Point pTemp;

            if (corners.cornerPoints.size() > 0) {
                pTemp = corners.cornerPoints.get(0);
            } else {
                pTemp = new Point(200.0, 200.0);
            }
            this.tl = pTemp;

            if (corners.cornerPoints.size() > 1) {
                pTemp = corners.cornerPoints.get(1);
            } else {
                pTemp = new Point(displayMetrics.widthPixels - 200.0, 200.0);
            }
            this.tr = pTemp;

            if (corners.cornerPoints.size() > 2) {
                pTemp = corners.cornerPoints.get(2);
            } else {
                pTemp = new Point(displayMetrics.widthPixels - 200.0, displayMetrics.heightPixels - 200.0);
            }
            this.br = pTemp;

            if (corners.cornerPoints.size() > 3) {
                pTemp = corners.cornerPoints.get(3);
            } else {
                pTemp = new Point(200.0, displayMetrics.heightPixels - 200.0);
            }
            this.bl = pTemp;
    ...
    private void init() {
        ...
        this.rectPaint.setColor(Color.rgb(175,0,255));
        ...
        this.circlePaint.setColor(Color.LTGRAY);
